...
def test_read_behavioral_csv_parses_and_converts(tmp_path):
    ...
    csv_path.write_text(
        "timestamp,sleep_hours,phone_usage,steps,screen_time_minutes,active_energy_kcal,calories_intake,protein_g,carbs_g,fat_g\n"
    ...
    assert row["phone_usage"] == 10
    assert row["steps"] == 1000
    assert row["screen_time_minutes"] == 30


def test_read_cognitive_csv_parses(tmp_path):
    csv_path = tmp_path / "cognitive.csv"
    csv_path.write_text(
        "timestamp,sequence_memory_score,reaction_time_ms,verbal_memory_words\n"
    ...
    row = df.iloc[0]
    assert pd.to_datetime("2025-11-17 08:00:00") == row["timestamp"]
    assert row["sequence_memory_score"] == 6
...
def test_merge_user_data_on_timestamp(tmp_path):
    ...
    assert "sequence_memory_score" in row
    assert row["sleep_hours"] == 7.5
    assert row["sequence_memory_score"] == 6
